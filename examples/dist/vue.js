...
(function (global, factory) {
   ...
   * is in that map.
   */
  // * 'key1,key2' 转换成 { key1: true, key2: true }
  // * 然后判断某个 key 是否在这里面
  function makeMap (
    str,
    expectsLowerCase
   ...
   */
  var camelizeRE = /-(\w)/g;
  // * 驼峰处理，如 vue-component 转换成 vueComponent
  ...
  // * 驼峰转换成连字符形式，如 vueComponent 转换成 vue-component
   ...
   */
  // * 类数组 转换成 数组，如 arguments 转换成 array
   ...
   */
  // * 由对象组成的数组转化成对象，如 [{ a: 1}, { b: 2}] 转换成 { a: 1, b: 2 }
   ...
   */
  // * 根据 modules 将 staticKeys 生成字符串，如 'staticClass,staticStyle'
   ...
   */
  // * 找到数组中的某个元素，相等是根据每个原始数据的值相等进行判断的
...

  {
    ...
    initProxy = function initProxy (vm) {
      if (hasProxy) {
        ...
        // * handers函数会根据 options.render._withStripped的不同执行不同的代理函数，
        // * 当使用类似webpack这样的打包工具时，通常会使用vue-loader插件进行模板的编译，
        // * 这个时候options.render是存在的，并且_withStripped的属性也会设置为true
  ...
  var normalizeEvent = cached(function (name) {
    // * 在监听事件的时候，会对修饰符进行处理，&~!等符号就是那个时候转换添加的
  ...
  function updateListeners (
    ...
    for (name in on) {
      ...
      if (isUndef(cur)) {
        ...
        // * 老事件不存在
        // * fns 不存在，那么 cur就是 fns =》 处理后cur.fns就存在了，且执行的时候可以批量执行
        // * 此处相当于把多个函数的执行封装成一个函数了
        if (isUndef(cur.fns)) {
        ...
        // * 老旧事件同时存在 ，old执行函数肯定是经过封装的，直接替换成现有的
        old.fns = cur;
        on[name] = old;
      }
    }
  ...
  // * 通过 inject key 向上寻找 provide 里的值
  ...
  function renderList (
    ...
    // * src/compiler/parser/index.js parseFor方法
    // * val： 被遍历的对象或数组
    // * render(
    // *  val: 遍历的每个元素值,
    // *  keyOrIndex: 如果是遍历的对象则为key，否则为index,
    // *  index: 遍历对象的index,
    // * )
  ...
  function initRender (vm) {
    ...
    // todo context 是父组件的实例？
    var renderContext = parentVnode && parentVnode.context;
    // * _renderChildren 是父组件调用子组件，在子组件之间的内容
    ...
    // render 函数并不是 vnode，因此需要递归生成vnode
  ...
  function lifecycleMixin (Vue) {
    Vue.prototype._update = function (vnode, hydrating) {
      ...
      // * __patch__ 方法在入口处注入了, platforms/web/runtime/index
      if (!prevVnode) {
        // initial render
      ...
      // todo https://juejin.cn/post/6862175878475546638
      // todo https://zhuanlan.zhihu.com/p/126552443
      // todo http://hcysun.me/2018/01/05/%E6%8E%A2%E7%B4%A2Vue%E9%AB%98%E9%98%B6%E7%BB%84%E4%BB%B6/
      // todo hoc 是什么？high order component
  ...
  function mountComponent (
    ...
    // * 实例化 render watcher, 此时会执行 getter(updateComponent) 方法
    // * 从而使得 data 将 updateComponent 作为依赖，每次数据改变时，触发 render
  ...
  Watcher.prototype.update = function update () {
    ...
    if (this.lazy) {
      // * 计算属性，每次设置或更新时设为true
      // * 因此每次计算属性依赖的数据发生变化，那么下次访问计算属性就会重新求值
      // * 如果用 method代替的话，那么每次render的时候都会去重新计算
      // * 如果用 watch 的话，watch其实也是一个依赖，它会被监听的数据收集，
      // * 当数据发生变化时，会触发watcher run方法，进而执行 watcher 回调
      this.dirty = true;
    } else if (this.sync) {
      this.run();
    } else {
  ...
  function createPatchFunction (backend) {
    ...
    // todo scope 设置？这里的 scope id 是什么，在哪里进行解析的？
    ...
    function updateChildren (parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {
          ...
          // * 如果新节点有 key, 在旧节点中找到对应 key 所在的 index
          // * 如果新节点没有 key, 那么在 旧节点中寻找 sameVnode
          idxInOld = isDef(newStartVnode.key)
            ? oldKeyToIdx[newStartVnode.key]
            : findIdxInOld(newStartVnode, oldCh, oldStartIdx, oldEndIdx);
          // * 如果不存在 index, 那么只能创建节点了
          if (isUndef(idxInOld)) { // New element
            ...
            if (sameVnode(vnodeToMove, newStartVnode)) {
              ...
              // * 设为 undefined 就会在遍历时被跳过
              oldCh[idxInOld] = undefined;
              canMove && nodeOps.insertBefore(parentElm, vnodeToMove.elm, oldStartVnode.elm);
            } else {
              // * 如果key相同，节点类型却不同，也只能重新创建了
    ...
    return function patch (oldVnode, vnode, hydrating, removeOnly) {
      ...
      if (isUndef(oldVnode)) {
        ...
        // * 更新 vnode 操作
        // * 如果老的 vnode 存在，判断是 vnode 还是 node
        var isRealElement = isDef(oldVnode.nodeType);
        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          // patch existing root node
          // * 如果都是 vnode，且当前节点相同，更新所有子节点（ diff 算法 ）
          patchVnode(oldVnode, vnode, insertedVnodeQueue, null, null, removeOnly);
        } else {
          // * 如果老节点不是 vnode ，或者 oldVnode 和 vnode 不是同类型
          // * 需要重新创建 新的节点
          if (isRealElement) {
            // * 如果老节点是 node 节点，则按空的 vnode 处理
          ...
          // destroy old node
          // * 如果有父节点，从父节点中移除当前老的 vnodes
          // * 如果不是父节点，那么需要被替换，销毁老 vnode
          if (isDef(parentElm)) {
  ...
  function addDirective (
    ...
    isDynamicArg,
    modifiers,
    // * range 代表该标识符在实际 template 中的 start, end
    range
  ) {
  ...
  function parse (
    ...
    function closeElement (element) {
      ...
      // * currentParent 存在，代表此标签在其他标签之内
      if (currentParent && !element.forbidden) {
        // * 如果有 elseif / else , 则需要前面的 children 有if
    ...
    parseHTML(template, {
      ...
      start: function start (tag, attrs, unary, start$1, end) {
        ...
        // * 文件路径：/platforms/web/compiler/modules/model
        // * 处理 input 标签 (checkbox / radio 等)
  ...
  // * 如果 v-for 存在，解析到 el 上
  // * { for, alias, iterator1, iterator2? } =》ForParseResult
  function processFor (el) {
    var exp;
    if ((exp = getAndRemoveAttr(el, 'v-for'))) {
      // * 解析，例如：(item, index) in data
    ...
    for (i = 0, l = list.length; i < l; i++) {
      ...
      if (dirRE.test(name)) {
        ...
        if (bindRE.test(name)) { // v-bind
          ...
          // * 如果是 v-on，绑定相关事件，包括 modifiers 的处理
  ...
  // * 将 input 标签分为 checkbox / radio / 其他三种
  ...
  function optimize (root, options) {
    if (!root) { return }
    // * /src/platforms/web/compiler/options.js 值为'staticClass,staticStyle'
  ...
  var CodegenState = function CodegenState (options) {
    ...
    // * platforms/web/compiler/modules/ style.js and class.js
  ...
  function genStatic (el, state) {
    ...
    // * 将能生成 vnode 的代码添加到 staticRenderFns 中
    // * 并且将生成的结果存储到 this._staticTrees 上
    // * 下一次可以通过 index 直接访问生成的结果
    // todo 代码与index的关联？
  ...
  function genOnce (el, state) {
    ...
    if (el.if && !el.ifProcessed) {
      return genIf(el, state)
    // * 如果在 for 循环中，判断 for循环是否有 key，生成 tree 并标记
